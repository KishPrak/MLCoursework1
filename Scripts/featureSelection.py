...
def get_pipeline(model_type, features):
    """Generates a pipeline tailored to the model and feature subset."""
    current_cat = [c for c in categorical_cols if c in features]
    current_num = [c for c in features if c not in categorical_cols]
    
    # Preprocessor: Scaling is mandatory for Ridge, MLP, and SVR
    ...
    if model_type == 'ridge':
        ...
        # RBF kernel is the standard for non-linear data
        ...
        model = VotingRegressor(
            ...
            weights=[1, 1] # You can tune these weights!
        )
        

    return Pipeline([("preprocessor", proc), ("model", model)])
